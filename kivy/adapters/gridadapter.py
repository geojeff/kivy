...
class GridAdapter(Adapter, EventDispatcher):
    ...
    def selection_mode_changed(self, *args):
        ...
        # Perhaps pop up a question, Deselect existing selection?
        # Because, here we do deselect.

        selection_removals = []

        for selected_view in self.selection:
            selected_view.deselect()
    ...
    def handle_selection(self, view, hold_dispatch=False, *args):
        ...
        op = 'deselect' if view in self.selection else 'select'

        if op == 'select':
            # Deselect existing selection when appropriate.
            if len(self.selection) > 0:
